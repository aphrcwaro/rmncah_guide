...
-   For the sub-national level, no UN projections are available, so we will access 4 methods only.
...
These first three steps above give the number of live births computed from pregnancies.
...
### Denominators based on estimated live births from DHIS2 and UN population sources

The idea is to use the estimated live birth from the two sources of population (DHIS2 and UN) and using the different parameters to derive the different denominators as listed in Table 3 above.

### Denominators derived from routine Health Facility data
...
### Selection of the best performing denominator
